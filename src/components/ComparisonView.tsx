...
export function ComparisonView({
  ...
  const hoursPercentChange =
    leftCalc.billableHours > 0
      ? ((rightCalc.billableHours - leftCalc.billableHours) /
          leftCalc.billableHours) *
        100
      : 0;

  // Determine winner based on net annual income
  const winner =
    netDifference > 0 ? "right" : netDifference < 0 ? "left" : null;

  // Calculate effective hourly rates
  const leftEffectiveHourly =
    leftCalc.billableHours > 0
      ? leftCalc.netAnnual / leftCalc.billableHours
      : 0;
  const rightEffectiveHourly =
    rightCalc.billableHours > 0
      ? rightCalc.netAnnual / rightCalc.billableHours
      : 0;
  const effectiveHourlyWinner =
    rightEffectiveHourly > leftEffectiveHourly
      ? "right"
      : rightEffectiveHourly < leftEffectiveHourly
        ? "left"
        : null;
  ...
  const leftName = getScenarioName(
    leftScenarioId,
    leftCompareState.title || "Scenario A",
  );
  const rightName = getScenarioName(
    rightScenarioId,
    rightCompareState.title || "Scenario B",
  );
      ...
      {leftScenarioId && rightScenarioId && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-[1fr,1fr] gap-4 text-center">
            <div
              className={`font-semibold text-lg py-2 rounded-lg ${winner === "left" ? "bg-emerald-500/10 border-l-4 border-emerald-500" : ""}`}
            >
              {leftName}
            </div>
            <div
              className={`font-semibold text-lg py-2 rounded-lg ${winner === "right" ? "bg-emerald-500/10 border-l-4 border-emerald-500" : ""}`}
            >
              {rightName}
            </div>
          </div>
          ...
          <ComparisonRow
            ...
            leftValue={formatCurrency(
              leftCalc.netAnnual,
              leftCompareState.currency,
            )}
            rightValue={formatCurrency(
              rightCalc.netAnnual,
              rightCompareState.currency,
            )}
          ...
          <ComparisonRow
            label="Gross Annual"
            leftValue={formatCurrency(
              leftCalc.grossAnnual,
              leftCompareState.currency,
            )}
            rightValue={formatCurrency(
              rightCalc.grossAnnual,
              rightCompareState.currency,
            )}
            ...
            winner={
              leftCalc.grossAnnual > rightCalc.grossAnnual
                ? "left"
                : leftCalc.grossAnnual < rightCalc.grossAnnual
                  ? "right"
                  : null
          ...
          <ComparisonRow
            ...
            winner={
              leftCalc.billableHours < rightCalc.billableHours
                ? "left"
                : leftCalc.billableHours > rightCalc.billableHours
                  ? "right"
                  : null
...
function ComparisonRow({
  ...
  const formattedAbsDiff = isCurrency
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[1fr,auto,1fr] gap-3 items-center">
        ...
        >
          <p
            className={`${valueClass} tabular-nums ${leftWins ? "text-emerald-600 dark:text-emerald-400" : ""}`}
          >
            {leftValue}
          </p>
          {/* Mini bar */}
        ...
        >
          <p
            className={`${valueClass} tabular-nums ${rightWins ? "text-emerald-600 dark:text-emerald-400" : ""}`}
          >
            {rightValue}
          </p>
          {/* Mini bar */}
...
function VerdictSummary({
  ...
  const winnerWorksMore =
    (winner === "right" && hoursDifference > 0) ||
    (winner === "left" && hoursDifference < 0);
  const winnerWorksFewer =
    (winner === "right" && hoursDifference < 0) ||
    (winner === "left" && hoursDifference > 0);

  // Effective hourly comparison
  const winnerEffectiveHourly =
    winner === "right" ? rightEffectiveHourly : leftEffectiveHourly;
  const loserEffectiveHourly =
    winner === "right" ? leftEffectiveHourly : rightEffectiveHourly;
  ...
  return (
    <div className="space-y-2 text-center">
      ...
      {effectiveHourlyDiff !== 0 && (
        <p className="text-sm text-muted-foreground">
          Effective hourly: {formatCurrency(winnerEffectiveHourly, currency)}/hr
          vs {formatCurrency(loserEffectiveHourly, currency)}/hr
          <span
            className={
              effectiveHourlyDiff > 0
                ? "text-emerald-600 dark:text-emerald-400"
          ...
          >
            {" "}
            ({effectiveHourlyDiff > 0 ? "+" : ""}
            {formatCurrency(effectiveHourlyDiff, currency)}/hr)
          </span>
        </p>
      )}
    </div>
