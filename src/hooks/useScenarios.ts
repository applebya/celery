...
export function generateScenarioName(state: CalculatorState): string {
  ...
  const regionName = country?.regions.find(r => r.code === state.region)?.name ?? state.region
...
export function useScenarios() {
  ...
  const saveScenario = useCallback((state: CalculatorState, id?: string): string => {
    const now = Date.now()
    ...
    setScenarios(prev => {
      ...
      if (id) {
        ...
        if (existing) {
          return prev.map(s =>
            s.id === id
              ? { ...s, state, updatedAt: now, name: s.name || generateScenarioName(state) }
              : s
          )
        }
      }
  ...
  const loadScenario = useCallback((id: string): CalculatorState | null => {
  ...
  const updateActiveScenario = useCallback((state: CalculatorState): void => {
