...
export function encodeStateToUrl(state: CalculatorState): string {
  ...
  if (state.calculationMode !== DEFAULT_STATE.calculationMode) {
    params.set(
      URL_PARAMS.mode,
      state.calculationMode === "salaryToHourly" ? "sth" : "hts",
    );
  }
  ...
  if (state.showCurrencyConversion !== DEFAULT_STATE.showCurrencyConversion) {
    params.set(
      URL_PARAMS.showConversion,
      state.showCurrencyConversion ? "1" : "0",
    );
  }
...
export function decodeUrlToState(search: string): Partial<CalculatorState> {
  ...
  const country = params.get(URL_PARAMS.country) as "CA" | "US" | "OTHER";
  if (country && ["CA", "US", "OTHER"].includes(country)) {
  ...
  if (showConversion !== null)
    state.showCurrencyConversion = showConversion === "1";
